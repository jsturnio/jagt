...
import { sampleWithRequiredData } from '../paquete.test-samples';
...
describe('Paquete Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 23723 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.paquetes()[0]).toEqual(expect.objectContaining({ id: 23723 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 23723 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.paquetes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 23723 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to paqueteService', () => {
      ...
      vi.spyOn(service, 'getPaqueteIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPaqueteIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.paquetesParams()).toMatchObject({ sort: ['id,desc'] });
  });
