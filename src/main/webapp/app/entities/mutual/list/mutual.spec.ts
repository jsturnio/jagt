...
import { sampleWithRequiredData } from '../mutual.test-samples';
...
describe('Mutual Management Component', () => {
  ...
  let comp: Mutual;
  let fixture: ComponentFixture<Mutual>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Mutual);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4576 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.mutuals()[0]).toEqual(expect.objectContaining({ id: 4576 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4576 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.mutuals()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4576 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to mutualService', () => {
      ...
      vi.spyOn(service, 'getMutualIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMutualIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.mutualsParams()).toMatchObject({ sort: ['id,desc'] });
  });
