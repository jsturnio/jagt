...
import { sampleWithRequiredData } from '../nomenclador.test-samples';
...
describe('Nomenclador Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Nomenclador;
  let fixture: ComponentFixture<Nomenclador>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Nomenclador);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 21757 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.nomencladors()[0]).toEqual(expect.objectContaining({ id: 21757 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 21757 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.nomencladors()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 21757 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to nomencladorService', () => {
      ...
      vi.spyOn(service, 'getNomencladorIdentifier');
      const id = comp.trackId(entity);
      expect(service.getNomencladorIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.nomencladorsParams()).toMatchObject({ sort: ['id,desc'] });
  });
