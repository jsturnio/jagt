...
import { sampleWithRequiredData } from '../bioquimico.test-samples';
...
describe('Bioquimico Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Bioquimico;
  let fixture: ComponentFixture<Bioquimico>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Bioquimico);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 24259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.bioquimicos()[0]).toEqual(expect.objectContaining({ id: 24259 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 24259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.bioquimicos()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 24259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to bioquimicoService', () => {
      ...
      vi.spyOn(service, 'getBioquimicoIdentifier');
      const id = comp.trackId(entity);
      expect(service.getBioquimicoIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.bioquimicosParams()).toMatchObject({ sort: ['id,desc'] });
  });
