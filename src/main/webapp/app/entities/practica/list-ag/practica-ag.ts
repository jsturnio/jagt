...
import { PracticaAgFilter } from './practica-ag-filter';
import { getPracticaColumnDefs } from './practica-grid.config';

ModuleRegistry.registerModules([AllCommunityModule]);

@Component({
  selector: 'jhi-practica-ag',
  templateUrl: './practica-ag.html',
  imports: [
    RouterLink,
    FormsModule,
    FontAwesomeModule,
...
export class PracticaAg implements OnInit, OnDestroy {
  ...
  columnDefs: ColDef[] = getPracticaColumnDefs(this.actionsCellRenderer.bind(this));
  ...
  onCellValueChanged(event: CellValueChangedEvent): void {
    const practica = event.data as IPractica;
  ...
  constructor() {
    ...
    (window as any).deletePractica = this.deletePracticaGlobal;
  }

  ngOnDestroy(): void {
    delete (window as any).deletePractica;
  }
  ...
  private deletePracticaGlobal = (id: number) => {
    const practica = this.practicas().find(o => o.id === id);
    if (practica) {
      this.delete(practica);
    }
  };
  ...
  actionsCellRenderer(params: any): string {
    const practica = params.data;
    return `
      <div class="d-flex gap-0">
        <a href="/practica/${practica.id}/view" class="btn  btn-sm icon-button icon-view" title="Ver" aria-label="Ver"></a>
        <a href="/practica/${practica.id}/edit" class="btn  btn-sm icon-button icon-edit" title="Editar"></a>
        <button class="btn  btn-sm icon-button icon-delete" title="Eliminar" onclick="window.deletePractica(${practica.id})"></button>
      </div>
    `;
  }
