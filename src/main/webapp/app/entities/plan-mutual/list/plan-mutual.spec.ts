...
import { sampleWithRequiredData } from '../plan-mutual.test-samples';
import { PlanMutualService } from '../service/plan-mutual.service';

import { PlanMutual } from './plan-mutual';

vi.useFakeTimers();

describe('PlanMutual Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PlanMutual;
  let fixture: ComponentFixture<PlanMutual>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PlanMutual);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 5137 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.planMutuals()[0]).toEqual(expect.objectContaining({ id: 5137 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 5137 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.planMutuals()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 5137 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to planMutualService', () => {
      ...
      vi.spyOn(service, 'getPlanMutualIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPlanMutualIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.planMutualsParams()).toMatchObject({ sort: ['id,desc'] });
  });
