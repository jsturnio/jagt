...
import { sampleWithRequiredData } from '../prestacion.test-samples';
...
describe('Prestacion Management Component', () => {
  ...
  let comp: Prestacion;
  let fixture: ComponentFixture<Prestacion>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Prestacion);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 17259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.prestacions()[0]).toEqual(expect.objectContaining({ id: 17259 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 17259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.prestacions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 17259 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to prestacionService', () => {
      ...
      vi.spyOn(service, 'getPrestacionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPrestacionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.prestacionsParams()).toMatchObject({ sort: ['id,desc'] });
  });
