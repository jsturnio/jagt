...
import { PrestacionDeleteDialog } from '../delete/prestacion-delete-dialog';
...
export class PrestacionAg implements OnInit, OnDestroy {
  ...
  readonly prestacions = signal<IPrestacion[]>([]);
  ...
  onCellValueChanged(event: CellValueChangedEvent): void {
    const prestacion = event.data as IPrestacion;
    this.prestacionService.update(prestacion).subscribe(() => {
      this.load();
    });
  }
  ...
  readonly isLoading = this.prestacionService.prestacionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.prestacionService.prestacionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.prestacions.set(this.fillComponentAttributesFromResponseBody([...this.prestacionService.prestacions()]));
    });
    ...
    (window as any).deletePrestacion = this.deletePrestacionGlobal;
  }

  ngOnDestroy(): void {
    delete (window as any).deletePrestacion;
  }

  trackId = (item: IPrestacion): number => this.prestacionService.getPrestacionIdentifier(item);
  ...
  private deletePrestacionGlobal = (id: number) => {
    const prestacion = this.prestacions().find(o => o.id === id);
    if (prestacion) {
      this.delete(prestacion);
    }
  };

  delete(prestacion: IPrestacion): void {
    const modalRef = this.modalService.open(PrestacionDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  actionsCellRenderer(params: any): string {
    const prestacion = params.data;
    return `
      <div class="d-flex gap-0">
        <a href="/prestacion/${prestacion.id}/view" class="btn  btn-sm icon-button icon-view" title="Ver" aria-label="Ver"></a>
        <a href="/prestacion/${prestacion.id}/edit" class="btn  btn-sm icon-button icon-edit" title="Editar"></a>
        <button class="btn  btn-sm icon-button icon-delete" title="Eliminar" onclick="window.deletePrestacion(${prestacion.id})"></button>
      </div>
    `;
  }
